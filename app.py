...
def modeling_tab():
    ...
    with st.expander("📚 How We Built These Models"):
        st.markdown("""
        **Data Preprocessing:**
        1. 🗑️ **Removed customerID** - Not predictive
        2. 🔢 **Converted TotalCharges** to numeric, median imputation for missing values
        3. 🏷️ **Automatic feature detection** - Separated categorical and numerical features
...
def explainability_tab():
    ...
    if st.session_state.models is None:
        st.warning("⚠️ Please train models first in the Modeling tab.")
        return
    ...
    if model_name in models:
        ...
        try:
            ...
            **What SHAP would show:**
            ...
            st.warning(f"⚠️ Could not generate SHAP plots: {str(e)}")
...
def insights_tab():
    ...
    if st.session_state.models is None:
        st.warning("⚠️ Please train models first to see insights.")
        return
    ...
    with st.expander("📖 Technical Terms Explained"):
        ...
        **🏷️ One-Hot Encoding**: Converts categorical variables into binary columns (0s and 1s)
